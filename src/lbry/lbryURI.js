...
module.exports.parseURI = (url, requireProto = false) => {
  ...
  if (requireProto && !proto) {
    throw new Error("LBRY URLs must include a protocol prefix (lbry://).");
  }

  // Validate and process name
  if (!streamNameOrChannelName) {
    throw new Error("URL does not include name.");
  }
  ...
  const isChannel =
    streamNameOrChannelName.startsWith("@") && !possibleStreamName;
  ...
  if (includesChannel) {
    ...
    if (channelName.length < channelNameMinLength) {
      throw new Error(
        `Channel names must be at least ${channelNameMinLength} characters.`
      );
    }
  }
...
const parseURIModifier = (modSeperator, modValue) => {
  let claimId;
  let claimSequence;
  let bidPosition;
  ...
  if (modSeperator) {
    if (!modValue) {
      throw new Error(`No modifier provided after separator ${modSeperator}`);
    }
...
module.exports.buildURI = (
  ...
  if (!isProduction) {
    if (claimId) {
      console.error(
        "'claimId' should no longer be used. Use 'streamClaimId' or 'channelClaimId' instead"
      );
    }
    if (claimName) {
      console.error(
    ...
    if (contentName) {
      console.error(
        "'contentName' should no longer be used. Use 'streamName' instead"
      );
    }
  }
  ...
  const primaryClaimName =
    claimName || contentName || formattedChannelName || streamName;
  const primaryClaimId =
    claimId || (formattedChannelName ? channelClaimId : streamClaimId);
...
module.exports.normalizeURI = (URL) => {
  const {
    streamName,
    streamClaimId,
    channelName,
...
module.exports.isNameValid = (claimName) => {
  return !regexInvalidURI.test(claimName);
};

module.exports.isURIClaimable = (URL) => {
  let parts;
  try {
...
module.exports.convertToShareLink = (URL) => {
  const {
    streamName,
    streamClaimId,
    channelName,
